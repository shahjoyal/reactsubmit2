...
export default function BunkerDiagram({
  ...
  // changed: store input in tonnes, convert to percent on add
  ...
  // confirmAdd converts tonnes -> percent (based on bunkerCapacity) and keeps previous behaviour
  async function confirmAdd() {
    const b = addOpen.bunkerIndex;
    if (b === null) return;

    // parse tonnes input
    let tNum = Number(tonnes || 0);
    ...
    if (tNum <= 0) {
      alert('Enter tonnes greater than 0.');
      return;
    }
    if (!bunkerCapacity || bunkerCapacity <= 0) {
      alert('Bunker capacity is not set or is zero, cannot compute percent from tonnes.');
      return;
    }

    // compute percent from tonnes
    let pctNum = (tNum / Number(bunkerCapacity)) * 100;
    // clamp percent range
    if (!isFinite(pctNum)) pctNum = 0;
    if (pctNum < 0) pctNum = 0;
    if (pctNum > 100) pctNum = 100;

    // compute existing percent sum in this bunker
    ...
    const remainingPercent = Math.max(0, 100 - existingSum);
    const remainingTonnes = (Number(bunkerCapacity) * remainingPercent) / 100;

    // If user-entered tonnes exceed remaining, reduce to remaining and inform
    if (tNum > remainingTonnes) {
      tNum = remainingTonnes;
      pctNum = remainingPercent;
      alert(`Tonnes reduced to ${tNum.toFixed(2)} (which is ${pctNum.toFixed(2)}%) to not exceed 100% in this bunker.`);
    }
    ...
    const layer = {
      ...
      coal: coalName,
      // store percent as before (business logic elsewhere continues to use percent)
      percent: pctNum,
      // if you want to keep tonnes on the layer too, store it (optional)
      tonnes: Number(tNum),
        ...
        <div className="popup">
          <div className="popup-content" role="dialog" aria-modal="true">
            ...
            <div className="popup-row">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (v === '') { setTonnes(''); if (typeof onUserEdit === 'function') onUserEdit('clientBunkers'); return; }
                  let n = Number(v);
                  if (isNaN(n)) n = 0;
                  if (n < 0) n = 0;
                  // optional: cap input to bunkerCapacity (not required because confirmAdd clamps)
                  if (Number(bunkerCapacity) > 0 && n > Number(bunkerCapacity)) n = Number(bunkerCapacity);
                  setTonnes(String(n));
                  if (typeof onUserEdit === 'function') onUserEdit('clientBunkers');
                }}
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              ...
              {/* disable Add if no coal selected or computed percent <= 0 */}
              <button
                className="btn btn-save"
                onClick={confirmAdd}
                disabled={!selectedCoalId || ((Number(bunkerCapacity) > 0 ? (Number(tonnes || 0) / Number(bunkerCapacity) * 100) : 0) <= 0)}
              >
                Add
              </button>
            </div>

...
// returns number of seconds from now until the given "HH:MM" time.
// if hhmm is falsy or invalid, returns null.
// If target time is earlier or equal to now, treats it as next day.
function computeSecondsUntil(hhmm) {
  ...
  const hours = Math.max(0, Math.min(23, hh));
  ...
  // if target is <= now, assume next day
...
function formatDelayLabelFromSeconds(sec) {
  if (sec == null) return '—';
  if (sec <= 0) return 'Starts now';
...
export default function BunkerDiagram({
  ...
  const [time, setTime] = useState(''); // time state for popup (HH:MM)
  ...
  function openCoalPopup(i) {
    setSelectedCoalId('');
    setPercent('');
    setTime(nowHHMM()); // default to current time (HH:MM)
  ...
  function showTooltip(ev, layer) {
    ...
    // include start delay if present
    const startLabel = layer && (layer.startDelayLabel || (layer.time ? formatDelayLabelFromSeconds(computeSecondsUntil(layer.time)) : undefined));
    tip.innerHTML = `<div style="font-weight:700;margin-bottom:6px">${escapeHtml(name)}</div>
      ...
      ${startLabel ? `<div style="margin-top:6px;display:flex;justify-content:space-between"><span style="opacity:0.85">Starts In</span><strong>${escapeHtml(startLabel)}</strong></div>` : ''}`;
  ...
  // confirmAdd keeps same behaviour, but now computes start delay from the popup time
  async function confirmAdd() {
    ...
    // compute seconds until chosen start time (HH:MM). if no time chosen, will be null
    const startInSeconds = computeSecondsUntil(time); // may be null
    let startDelayHours = null;
    let startDelayMinutes = null;
    let startDelayLabel = undefined;
    if (startInSeconds != null) {
      startDelayHours = Math.floor(startInSeconds / 3600);
      startDelayMinutes = Math.floor((startInSeconds % 3600) / 60);
      startDelayLabel = `${startDelayHours}h ${startDelayMinutes}m`;
    }
    ...
    const layer = {
      ...
      color,
      time: time || undefined, // "HH:MM" string
      startInSeconds: startInSeconds != null ? startInSeconds : undefined,
      startDelayHours: startDelayHours != null ? startDelayHours : undefined,
      startDelayMinutes: startDelayMinutes != null ? startDelayMinutes : undefined,
      startDelayLabel: startDelayLabel
    };
  ...
  // compute live start label for the popup (recomputed each render)
  const popupStartSeconds = computeSecondsUntil(time);
  const popupStartLabel = popupStartSeconds != null ? formatDelayLabelFromSeconds(popupStartSeconds) : '—';
      ...
      {addOpen.open && (
        <div className="popup">
          <div className="popup-content" role="dialog" aria-modal="true">
            ...
            {/* Time input (hours:minutes only) */}
            <div className="popup-row" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label>Start Time</label>
              <input
                type="time"
                value={time}
                onChange={(e) => {
                  setTime(e.target.value); // e.target.value is "HH:MM"
                  if (typeof onUserEdit === 'function') onUserEdit('clientBunkers');
                }}
                placeholder="HH:MM"
              />
              <div style={{ fontSize: 13, color: '#555' }}>
                Starts in: <strong style={{ color: '#111' }}>{popupStartLabel}</strong>
              </div>
            </div>
